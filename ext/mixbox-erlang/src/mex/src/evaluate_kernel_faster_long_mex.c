...
/* Use long doubles for computing the kernel (this is the /only/ difference to evaluate_kernel_faster_mex) */
#define LONG_SUBKERNEL_COMPUTATIONS

/* Include source code (a bit of a hack to reuse code in multiple MEX files with compiling library files) */
#include "evaluate_kernel_faster.c"
        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    if(ComputeGradient){
        ...
        evaluate_kernel_faster(t, c, a, nz, M, N, ComputeGradient, ComputeHessian,
            alpha, dalpha, d2alpha);
    } else {
    
        /* Do the actual computations in a subroutine */
        evaluate_kernel_faster_without_derivatives(t, c, a, nz, M, N, ComputeGradient, ComputeHessian,
            alpha);
